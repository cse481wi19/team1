...
def transform_to_pose(matrix):
    ...
    # Lab 24 (Fetch) / Lab 22B (Kuri): Transform Arithmetic
    # The translation vector is the last column of the matrix
    pose.position.x = float(matrix[0][-1])
    pose.position.y = float(matrix[1][-1])
    pose.position.z = float(matrix[2][-1])

    # Convert the matrix to the quaternion and extract [x,y,z,w] values
